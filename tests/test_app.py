...
check("dashboard has no rotation controls", r.text.count("/toggle") == 0)
check("dashboard no db error", "Database error" not in r.text)
tok = csrf_of(r.text)
r_rot = c.get("/rotation")
check("rotation page 200", r_rot.status_code == 200, r_rot.status_code)
check("rotation page 14 rows", r_rot.text.count("/toggle") == 14, r_rot.text.count("/toggle"))
check("rotation page no db error", "Database error" not in r_rot.text)
r_cf = c.get("/country-filter")
check("country-filter page 200", r_cf.status_code == 200, r_cf.status_code)
check("country-filter no db error", "Database error" not in r_cf.text)
for _p in ("/rotation", "/country-filter", "/detections"):
    check(f"{_p} requires login", new_client().get(_p).status_code == 303)
